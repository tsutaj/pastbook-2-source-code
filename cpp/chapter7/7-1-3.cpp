...
int main() {
    ...
    vector<long long> B;
    B.emplace_back(0);
    vector<long long> S; // A の累積和
    S.emplace_back(0);

    long long sumA = 0, maxB = 0;
    for(int i = 0; i < N; i++) {
        int a, b;
        cin >> a >> b;
        sumA += a;
    ...
    LazySegmentTree<long long, long long> segtree(
        M + 1, INF, INF, [](auto a, auto b) { return min(a, b); },
        [](auto a, auto b) { return min(a, b); },
        [](auto a, auto b) { return min(a, b); });
    segtree.update(0, 1, 0);

    for(int i = 0; i < M - 1; i++) {
        long long cost = segtree.fold(i, i + 1);
        long long money = S[i + 1] - cost - B[i];
        // どの仕事までをこなせるか
        int ubound = upper_bound(B.begin(), B.end(), money) - B.begin();
        // 仕事をこなせる範囲で動的計画法の配列を更新
        segtree.update(i + 1, ubound, cost + B[i]);
    }

    // 最終日のコンピュータの値段を反映させる
    long long ans = sumA - segtree.fold(M - 1, M) - B[M - 1];
    if(ans < 0) {
        puts("-1");
    } else {
        cout << ans << endl;
