...
using OperatorTp = pair<int, int>;

// セグメント木に使用する関数
// node は (値, 区間の長さ) を、lazy は (b, c) を持っている
auto combine_node = [](MonoidTp lhs, MonoidTp rhs) {
    int ret0 = (lhs.first + rhs.first) % MOD;
    int ret1 = lhs.second + rhs.second;
    return make_pair(ret0, ret1);
};

auto combine_lazy = [](OperatorTp lhs, OperatorTp rhs) {
    return make_pair(int(1LL * lhs.first * rhs.first % MOD),
                     int((1LL * lhs.second * rhs.first + rhs.second) % MOD));
};

auto reflect = [](MonoidTp node, OperatorTp lazy) {
    return make_pair(
        int((1LL * lazy.first * node.first + 1LL * lazy.second * node.second) %
            MOD),
        node.second);
};
...
int main() {
    ...
    LazySegmentTree<MonoidTp, OperatorTp> segtree(N, make_pair(0, 0),
                                                  make_pair(1, 0), combine_node,
                                                  combine_lazy, reflect);
    ...
    for(int i = 0; i < Q; i++) {
        ...
        if(q == 0) {
            int l, r, b, c;
            cin >> l >> r >> b >> c;
            segtree.update(l, r, make_pair(b, c));
        } else if(q == 1) {
            int l, r;
            cin >> l >> r;
            cout << segtree.fold(l, r).first << endl;
        }
    }
    return 0;
}
