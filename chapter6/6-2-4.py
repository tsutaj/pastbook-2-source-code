...
def combine_node(lhs, rhs):
    return (lhs[0] + rhs[0], lhs[1] + rhs[1], lhs[2] + rhs[2] + lhs[1] * rhs[0])
...
segtree = LazySegmentTree(N, (0, 0, 0), 0, combine_node, combine_lazy, reflect)

# 数列の初期化 (要素を 1 つずつ見るとき、転倒数は 0 である)
B = [(A[i] == 0, A[i] == 1, 0) for i in range(N)]
segtree.build(B)
